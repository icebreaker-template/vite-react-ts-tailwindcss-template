...
// constants
export const STATUS_ORIGINAL = 0; // The default status, locating at the original position.
export const STATUS_RELEASED = 1; // The released status, locating at somewhere on document but not default one.
export const STATUS_FIXED = 2; // The sticky status, locating fixed to the top or the bottom of screen.
...
export function useSticky(props) {
  ...
  const needTriggerUpdate =
    propsRef.current.top !== props.top ||
    propsRef.current.bottomBoundary !== props.bottomBoundary; // if the top or bottomBoundary props were changed, then trigger the update
      ...
      if (state.height > winHeight - state.top) {
        ...
        switch (state.status) {
          case STATUS_ORIGINAL:
            ...
            dataRef.current.stickyBottom =
              dataRef.current.stickyTop + state.height;
          ...
          case STATUS_FIXED:
            ...
            if (delta > 0 && pos === state.top) {
              ...
              const deltaHeight = pos + height - state.bottom;
  ...
  useEffect(() => {
    if (propsRef.current.onStateChange) {
